...
template <typename Dtype>
void MultilabelDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>&
...
template <typename Dtype>
void MultilabelDataLayer<Dtype>::ShuffleVideos() {
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    trans_time += timer.MicroSeconds();
		...
		for (int i = 0; i < label_length; i++) {
			prefetch_label[item_id*label_length + i] = lines_[lines_id_].label[i];
		}
    // go to the next iter
    lines_id_++;
