...
#include "caffe/layers/multilabel_crop_data_layer.hpp"
...
MultilabelCropDataLayer<Dtype>::~MultilabelCropDataLayer<Dtype>() {
  this->StopInternalThread();
}

template <typename Dtype>
void MultilabelCropDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>&
  ...
  while (std::getline(infile, line)) {
    multilet video_and_label;
		...
		pos = line.find(' ');
		video_and_label.frame = atoi(line.substr(0, pos).c_str());
		line = line.substr(pos + 1);
		video_and_label.label.clear();
		while ((pos = line.find(' ')) != string::npos) {
			label = float(atof(line.substr(0, pos).c_str()));
  ...
  std::vector<cv::Mat> cv_imgs;
	vector<int> crop;
	crop.clear();
	crop.push_back(int(lines_[lines_id_].label[0]));
	crop.push_back(int(lines_[lines_id_].label[2]));
	crop.push_back(int(lines_[lines_id_].label[1]));
	crop.push_back(int(lines_[lines_id_].label[3]));
  bool read_video_result = ReadVideoToCVMatCrop(root_folder +
                                            lines_[lines_id_].name,
                                            lines_[lines_id_].frame,
																						crop[0], crop[1], crop[2], crop[3],
                                            new_length, new_height, new_width,
                                            is_color,
                                            &cv_imgs);
  CHECK(read_video_result) << "Could not load " << lines_[lines_id_].name <<
                              " at frame " << lines_[lines_id_].frame << ".";
  ...
  // label
	int label_size = lines_[0].label.size() - 4;
  vector<int> label_shape;
	label_shape.clear();
	label_shape.push_back(batch_size);
	label_shape.push_back(label_size);
	LOG(INFO) << "label size: " << label_shape[0] << ","
      << label_shape[1];
...
template <typename Dtype>
void MultilabelCropDataLayer<Dtype>::ShuffleVideos() {
...
template <typename Dtype>
void MultilabelCropDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  Dtype* prefetch_label = batch->label_.mutable_cpu_data();
	int label_length = batch->label_.shape(1);
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    trans_time += timer.MicroSeconds();
		...
		for (int i = 0; i < label_length; i++) {
			prefetch_label[item_id*label_length + i] = lines_[lines_id_].label[i+4];
		}
    // go to the next iter
    lines_id_++;
...
INSTANTIATE_CLASS(MultilabelCropDataLayer);
REGISTER_LAYER_CLASS(MultilabelCropData);
