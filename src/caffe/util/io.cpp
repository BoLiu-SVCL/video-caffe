...
bool ReadVideoToCVMat(const string& path,
  ...
  // In case of a directory with extracted frames within
    ...
    for (int i = start_frame; i <= end_frame; ++i) {
      snprintf(image_filename, sizeof(image_filename), "%s/%05d.jpg",
               path.c_str(), i);
...
bool ReadVideoToCVMatCrop(const string& path, const int start_frame,
  ...
  if (is_video_file) {
    ...
    for (size_t i = start_frame; i <= end_frame; ++i) {
      ...
      if (height > 0 && width > 0) {
        cv::resize(cv_img_origin(cv::Range(crop_upper-1, crop_lower), cv::Range(crop_left-1, crop_right)),
						cv_img,
